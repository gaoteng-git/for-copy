#!/usr/bin/env python3
"""
验证 embedding 计费：通过 Router **prod** 发真实 /v1/embeddings 请求，
累计响应里的 usage.prompt_tokens，按配置的单价算出"预期扣费"，
拿去跟 prod 账户 UI 上实际扣掉的余额对比。

⚠️ 这是 prod，真实扣真实账户的钱，不是 staging 测试环境。

⚠️ ROUTER_API_KEY 要换成 prod 账户的 key，不能沿用之前 staging 那把——
staging（router-api-staging.0g.ai）和 prod（router-api.0g.ai）是两个独立
部署（各自独立的 GCP 项目 + MySQL 数据库，见 0g-router CLAUDE.md 的部署
架构一节），账户体系不共享。staging 的 key 在 prod 这边查不到对应记录，
会 401；反过来 prod 的 key 也不能拿去 staging 用。去 prod 前端（pc.0g.ai）
登录后在 API Key 管理页新建一把 prod 的 key，再 export 这把新的。

为什么要凑到 >0.1 0G：账户余额 UI 只显示 2 位小数，扣费太小会被舍入吃掉，
看不出账目对不对（具体阈值按你 prod 账户当前余额自己调整 TARGET_TOTAL_TOKENS）。

**这一步脚本做不到，需要你自己手动做**：
  1. 运行脚本前，先在 prod 账户页面记下当前余额。
...
用法：
    export ROUTER_API_KEY=sk-...   # prod 账户的 key，不是 staging 那把
    python3 test-embedding-usage-billing.py
"""
import json
import os
import sys
...
ROUTER_URL = "https://router-api.0g.ai/v1/embeddings"
MODEL = "qwen3.7-text-embedding"

# 实测取自 prod GET /v1/models 的 pricing_usd.prompt = "0.0000000735"
# （2026-09-09 核对；deploy 仓库里那份 user_config 写的是 "0.0777"，已经
# 是过期值，本地 clone 没有 push 权限拿不到最新的价格更新提交，实时
# /v1/models 才是权威来源，见 qwen3.7-text-embedding-admission-report-prod.md）
PRICE_USD_PER_M_TOKENS = 0.0735
...
def main():
    ...
    print()
    print("现在去 prod 账户页面刷新余额，用「运行前余额 - 运行后余额」")
